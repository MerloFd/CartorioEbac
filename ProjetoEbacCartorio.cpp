...
int ESC() //function de saida com ESC
{
    ...
    if (getch() == 27) // Le o caractere pressionado. 27 é o código ASCII para a tecla "ESC"
	{
		system ("cls");
    	return 1; // Retorna 1 para indicar que o usuário deseja voltar ao menu caso pressione a tecla "ESC"
    } else {
    	system ("cls");
...
int consulta() // Função de consulta
{
	...
	while(1)
	{
		...
		scanf("%s",cpf); // armazena a resposta na string cpf 
			...
			char arquivo[40]; // variavel que armazena o nome do nosso arquivo, que é igual o cpf
		...
		if (file != NULL) // se o arquivo existir, faca isso
		{
			...
			{
				char line [40]; // linha que vai trazer as informações do arquivo para tela. Tem o mesmo tamanho do arquivo
